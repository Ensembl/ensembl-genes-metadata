      ...
      foreach my $experiment (@{$study->{$sample}}) {
        ...
        foreach my $file (@files) {
          ...
          if ($samples->{$sample}->{tax_id} eq ''){
             $samples->{$sample}->{tax_id} = $self->param('sp_id');
          }
          ...
          if($self->param('read_type') eq 'short_read') {
			  print FH sprintf("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s, %s\t%s\t%s\t%s\t%s\n",,
				lc($sample_name),
				$experiment->{run_accession},
				$experiment->{library_layout} eq 'PAIRED' ? 1 : 0,
				$samples->{$sample}->{tax_id},
				$filename,
				$self->param('is_mate_1'),
		...
		print FH sprintf("%s\t%s\t%s\t%s\t%s, %s\t%s\t%s\t%s\t%s\n",
		lc($sample_name),
		$filename,
		$experiment->{library_layout} eq 'PAIRED' ? 1 : 2,
		$self->param('sp_id'),
                $study_accession,
                $sample,
		$file,
		$checksums[$index],
	          ...
	          print LR sprintf("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s, %s, %s\t%s\t%s\t%s\t%s\n",
				lc($sample_name),
				$experiment->{run_accession},
				$experiment->{library_layout} eq 'PAIRED' ? 1 : 2,
				$samples->{$sample}->{tax_id},

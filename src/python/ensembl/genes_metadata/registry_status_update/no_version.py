...
def get_status_updates(merged_df):
    ...
    updated_df = updated_df[
        [
            'gca_accession', 'genebuild_status_id', 'gb_status_new', 'release_date_new',
...
def main(password, test, apply_method, old_registry, apply_old):
    ...
    logger.info(f"Adding missing methods")
    ...
    if production_status.empty:
        ...
        if (dups > 1).any():
            ...
            # remove ALL rows that are part of a duplicate group
            dup_rows = gb_status[gb_status.duplicated(subset=merge_keys, keep=False)]
            logger.info("Removing duplicated GCAs from df:\n%s", dup_rows[merge_keys])

            gb_status = gb_status.drop(dup_rows.index)

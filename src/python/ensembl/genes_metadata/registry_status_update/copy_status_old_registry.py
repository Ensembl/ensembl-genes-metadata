...
def fetch_status_old_registry():
    ...
    old_gb_status = old_gb_status[
        old_gb_status["genebuilder"].isin(current_genebuilders)
    ].copy()

    old_gb_status["date_started"] = pd.to_datetime(old_gb_status["date_started"])
    logger.info(
        f"Found {len(old_gb_status)} entries in the old genebuild_status table."
    )
    old_reg_df_latest = (
        old_gb_status.sort_values("date_started", ascending=False)
        .drop_duplicates(subset="gca_accession", keep="first")
        .reset_index(drop=True)
    )
    logger.info(
        f"Keeping only the latest entries from old registry per GCA. Remaining entries: {len(old_reg_df_latest)}"
    )
...
def delete_entries_not_production_db(filtered_old):
    ...
    not_in_prod = (filtered_old["gb_status"].isin(["live", "handed_over"])) & (
...
def check_status_update_old_registry(gb_status, old_reg_df_latest):
    ...
    merged["gb_status_new_mapped"] = (
        merged["gb_status_new"].map(status_map).fillna(merged["gb_status_new"])
    )

    # 3. Apply final filtering
    filtered = merged[
        merged["mapped_status"].isin(progressed_set)  # old registry progressed
        & merged["gb_status_new_mapped"].isin(
            in_progress_set
        )  # new registry still in progress
        & (
            merged["mapped_status"] != merged["gb_status_new_mapped"]
...
def add_entries_from_old_registry(gb_status):
    ...
    logger.info(
        f"Found {len(existing_accessions)} entries in the new genebuild_status table."
    )
    filtered_old = old_reg_df_latest[
        ~old_reg_df_latest["gca_accession"].isin(existing_accessions)
    ].copy()
    logger.info(
        f"Filtered old registry: {len(filtered_old)} rows remain after removing existing accessions"
    )
    ...
    filtered_old["gb_status"] = (
        filtered_old["gb_status"].map(status_map).fillna(filtered_old["gb_status"])
    )

    # Rename columns
    filtered_old = filtered_old.rename(
        columns={
            "annotation_source": "annotation_method",
            "date_completed": "date_status_update",
        }
    )
...
def find_assembly_id(gb_status):
    ...
    if filtered_old is None or filtered_old.empty:
        logger.info("No old registry entries found to insert.")
        return None
    ...
    if len(gca_tuple) == 0:
        logger.info("No GCA accessions to look up.")
        return filtered_old.assign(assembly_id=pd.NA)
...
def insert_entries_from_old_registry(password, gb_status, dry_old):
    assembly_id_merged = find_assembly_id(gb_status)
    if assembly_id_merged is None:
        return None

    # Keep only rows with missing assembly_id
    missing_assembly_df = assembly_id_merged[
        assembly_id_merged["assembly_id"].isnull()
    ][["gca_accession"]].copy()
    ...
    assembly_id_merged = assembly_id_merged[
        assembly_id_merged["assembly_id"].notnull()
    ].copy()
    logger.info(
        f"{len(assembly_id_merged)} rows remain after removing entries with missing assembly_id."
    )

    if dry_old:
        logger.info("Test mode: no updates applied from old registry.")
        logger.info(
            "Annotations that would be inserted to new registry from old:\n%s",
            assembly_id_merged.to_string(index=False),
        )
        return assembly_id_merged
    ...
    insert_query = (
        f"INSERT INTO genebuild_status ({columns_str}) VALUES ({placeholders})"
    )
    ...
    logger.info(
        f"Inserted {len(insert_merged_df)} rows from old registry to new registry."
    )

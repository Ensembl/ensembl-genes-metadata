...
sub default_options {
  ...
  return {
    ...
    'pipeline_db' => {
                       -dbname => 'transcriptomic_registry_pipeline',
                       -host   => $ENV{GBS1},
                       -port   => $ENV{GBP1},
                       -user   => $self->o('user_w'),
...
########################
    ...
    'pipe_db_name'              => 'transcriptomic_registry_fish',
    'registry_port'             => $self->o('pipe_db_port'), # port for general output db host
    'output_path'               => '/hps/nobackup/flicek/ensembl/genebuild/do1/transcriptomic_registry_fish/', # Lustre output dir. This will be the primary dir to house the assembly info and various things from analyses
    ...
    'get_assembly_script_path'  => $ENV{ENSCODE} . '/ensembl-genes-metadata/src/perl/get_assembly.pl',
   ...
   my ($self) = @_;
    return [
    	{ 
		 ...
		 },
		 {
			...
			-flow_into => { 
							...
							'A->1' => ['create_species_alignment_job'],#{'create_sample_merge_jobs' => {'inputfile' => '#inputfile#'}},
			}, 
		},
		{
		...
		-logic_name => 'generate_minimap2_jobs',
          ...
          {
          	  -logic_name => 'minimap2',
          	  -module     => 'Bio::EnsEMBL::Pipeline::Runnable::Minimap2',
          	  -parameters => {
          ...
          
          {
          	  ...
          	  -module     => 'Bio::EnsEMBL::Pipeline::Runnable::Minimap2',
          	  -parameters => {
                         genome_file => catfile($self->o('genome_path').'#species#/#accession#'. "/",  '#accession#'),
                         minimap2_path => $self->o('minimap2_path'),
                         input_dir => $self->o('output_path').'#species#'. "/fastq/",
                         output_dir => $self->o('output_path').'#species#/#accession#'.'/alignment',
                         samtools => $self->o('samtools_path'),

...
sub run{
  ...
  sub classify_rnaseq{
   ...
   #Since long read data is only utilised when available and not a determining factor for assembly annotation, we don't need to assign rnaseq status based on it

...
package HiveDownloadRNASeqFastqs;
...
sub is_rnaseq_classified{
  my ($self) = @_;
    ...
    my $sth;
    if ($_[2] == 1){
      $sth = $dba->dbc->prepare("select * from rnaseq_data_extra where ID = ? and species_id = ?");
    }
    else{
      $sth = $dba->dbc->prepare("select * from isoseq_data_extra where ID = ? and species_id = ?");
    }
 ...
 #Add downloaded fastq files to csv table in pipeline db
  my $dba = new Bio::EnsEMBL::DBSQL::DBAdaptor(
    ...
    if (defined($pair) and ($self->param('is_paired') ==  1)){
    	...
    	$sth->bind_param(6,$self->param('is_mate_1'));
    	$sth->bind_param(7,$self->param('read_length'));
    	$sth->bind_param(8,$self->param('is_13plus'));
    	$sth->bind_param(9,$self->param('CN'));
    	$sth->bind_param(10,$self->param('PL'));
    	$sth->bind_param(11,$self->param('DS'));
    	$sth->bind_param(12,$self->param('url'));
    	$sth->bind_param(13,$self->param('md5'));
    	$sth->bind_param(14,$self->param('species'));
        $sth->bind_param(15,$self->param('source_id'));
     }
     else{

...
package Bio::EnsEMBL::Pipeline::Runnable::CreateStarJobs;
...
sub fetch_input {
  ...
  my @results = $registry_adaptor->fetch_short_reads_by_species_id($self->param('sp_id'));
  my @output_ids;
  ...
  foreach my $sample_id (sort { $file_sizes->{$a} <=> $file_sizes->{$b} } keys %$file_sizes) {
    ...
    if($file_size + $total_size > $target_batch_size) {
      if(scalar(@$single_batch_array)){
        push(@{$all_batches},[$single_batch_array,$self->param('species'),$self->param('accession')]);
        $single_batch_array = [];
        $total_size = 0;
      }
    }

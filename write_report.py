...
def enrich_csv_with_paths(csv_path, base_dir, output_csv, merge_tissue, bam2bigWig, engine):
    base_dir = Path(base_dir).resolve()
    df = pd.read_csv(csv_path)
    accessions = df["run_accession"].dropna().unique().tolist()
    placeholders = ','.join(['%s'] * len(accessions))
    print("SONO QUI %s",base_dir)
    ...
    for idx, row in df.iterrows():
        ...
        biosample = str(row['sample_accession'])
        #sample_accession = str(row['sample_accession'])
        tissue =  str(row['tissue'])
        genome_index_dir =  Path(base_dir) / taxon_id / assembly_accession
        #print("Looking for .fna in:", list(genome_index_dir.glob("*.fna")))

        genome_file = list(genome_index_dir.glob("*.fna"))[0]
        ...
        if (paired):
            ...
            relative_minimap_index_genome=minimap_index_genome.relative_to(base_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base_dir", required=True, help="Base directory where outputs are stored")
    parser.add_argument("--out", default="enriched_output.csv", help="Output CSV filename")
    parser.add_argument("--merge_tissue", type=str, choices=['true', 'false'], default='false', required=False, help="option to diplay report fot merged tissue")
    parser.add_argument("--bam2bigWig", type=str, choices=['true', 'false'], default='false', required=False, help="option to diplay report fot merged tissue")

    args = parser.parse_args()
    engine = create_engine(
        f"mysql+pymysql://{args.user}:{args.password}@{args.host}:{args.port}/{args.database}"
    )
    enrich_csv_with_paths(args.csv, args.base_dir, args.out, args.merge_tissue, args.bam2bigWig, engine)

...
taxonomy = APIRouter(prefix="/taxonomy", tags=["taxonomy"], responses={404: {"description": "Not found"}})
...
async def assign_clade(request: Dict):
    ...
    try:
        if "lowest_taxon_id" not in request or "taxonomy_dict" not in request:
            raise HTTPException(
                status_code=400, detail="Request must include 'lowest_taxon_id' and 'taxonomy_dict'"
            )
        ...
        internal_clade, species_taxon_id, genus_taxon_id, pipeline = assign_clade_and_species(
            lowest_taxon_id, clade_data, taxonomy_dict, chordata_taxon_id, human_taxon_id
        )
...
async def batch_assign_clade(request: Dict):
    ...
    try:
        if "taxon_ids" not in request or "taxonomy_dict" not in request:
            raise HTTPException(
                status_code=400, detail="Request must include 'taxon_ids' and 'taxonomy_dict'"
            )
        ...
        for taxon_id in taxon_ids:
            internal_clade, species_taxon_id, genus_taxon_id, pipeline = assign_clade_and_species(
                taxon_id, clade_data, taxonomy_dict, chordata_taxon_id, human_taxon_id
            )

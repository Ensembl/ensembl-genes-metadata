...
def get_ready_to_ho(genebuilder):
    try:
        ...
        collapsed = df.groupby("gca", as_index=False).agg(
            {
        ...
        list_data = check_data_df.loc[
            (date_series < six_months_ago), ["gca", "scientific_name", "gb_status"]
        ]
        ...
        return (
            ...
            count_pending,
            (
                pd.DataFrame(list_data).to_dict(orient="records")
                if not list_data.empty
                else []
            ),
            (
                pd.DataFrame(list_pending).to_dict(orient="records")
                if not list_pending.empty
                else []
            ),
        )

...
def get_metadata_from_registry(taxon_ids: list[int]) -> pd.DataFrame:
    """Retrieve transcriptomics registry metadata for one or more taxon IDs."""
    if not taxon_ids:
        logging.warning("No taxon IDs provided for transcriptomic assessment.")
        return pd.DataFrame(
            columns=[
        ...
        query = f"""
            SELECT m.taxon_id,
                   ...
                   r.tissue_prediction,
                    ...
                    a.percentage_reads_mapped_to_multiple_loci,
                    a.percentage_reads_unmapped_too_short
            FROM meta m
            JOIN run r ON m.taxon_id = r.taxon_id
            JOIN align a on r.run_id = a.run_id
            WHERE m.taxon_id IN ({placeholders})
